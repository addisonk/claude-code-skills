...
1. **Pick the journeys first.** A flow is something a real user *does*. Examples: "Sign up and finish onboarding", "Make a first purchase", "Change notification settings", "Recover a forgotten password". Aim for **3–6 flows** that together cover the product's reason for existing. If you can't find user-visible flows by reading routes, screens, and entry points — **ask the user** which journeys matter. Do not invent flows from backend code.
